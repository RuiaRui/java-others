...
public class RecommendImpl implements Recommend {
    ...
    private static ChineseSegment segment = new ChineseSegmentImpl();
     ...
     */
    private List<String> GetKeyWords(StockInfo[] stocks) {
        //每个stock的content分词
        ArrayList<List<String>> contentWords = new ArrayList<>();
        for (StockInfo stock : stocks) {
            contentWords.add(segment.getContentFromInput(new StockInfo[]{stock}));
        }
     ...
     * @return frequency
     */
    private int[][] FrequencyCalculator(List<String> keywords, StockInfo[] stocks) {
        ...
        for (StockInfo s : stocks) {
            ...
            List<String> words = segment.getContentFromInput(new StockInfo[]{s});
    ...
    private Double CosCalculator(int[] array1, int[] array2) {
        int numerator = 0;
    ...
    private double[][] CosSimilarityCalculator(int[][] frequentMatrix) {
